...
final class Model_Toolbox extends Model {
	...
	public function getFiles($folderId){
		$params = array();
		$return = array();
		$sql = 'SELECT * FROM `#__Toolbox` WHERE `#__Toolbox`.`status` = 1 ';
		$sql .= 'AND `#__Toolbox`.`folder_id` = ?';
		$sql .= ' ORDER BY `#__Toolbox`.`aid` ASC' ;
		$result = $this->execute($sql, $folderId);
		foreach ($result as $files){
			$return[$files['id']] = $files;
			$info = new SplFileInfo($files['path']);
			$return[$files['id']]['filetype'] = $info->getExtension();
		}
		return $return;
	}

	public function getFolders($propertyId = NULL, $divisionId = NULL){
		$params = array();
		$return = array();
		$sql = 'SELECT * FROM `#__Folder` WHERE `#__Folder`.`status` = 1 ';
		if ($divisionId != ''){
		...
		$sql .= ' ORDER BY `#__Folder`.`aid` ASC' ;
		$result = $this->execute($sql, $params);
		foreach ($result as $folders){
			$return[$folders['id']] = $folders;
		}
		return $return;
	}
